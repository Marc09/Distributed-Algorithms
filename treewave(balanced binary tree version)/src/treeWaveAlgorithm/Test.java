...
//Balanced BinaryTree version

import java.util.Random;

public class Test {
	
	static Node root = new Node(4);
		...
		if (node.getDecide()){
			...
			if (node.recCounter(node) == 1){
				...
				if (node.isLeaf()){
					...
					if (node.getLeftChild() != null && node.nodeRecNumber.get(node.getLeftChild()).equals(false) && !node.getLeftChild().getDecide()){
					...
					if (node.getRightChild() != null && node.nodeRecNumber.get(node.getRightChild()).equals(false) && !node.getRightChild().getDecide()){
			...
			for (int j = 0; j < r; j++){
				...
				switch(ranNode){
						...
						case 2: waveAlgorithm(n1);
								break;
						case 6: waveAlgorithm(n2);
								break;
						case 1: waveAlgorithm(n3);
								break;
						case 3: waveAlgorithm(n4);
								break;
						case 5: waveAlgorithm(n5);
								break;
