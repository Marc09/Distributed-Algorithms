		...
		if (node.getDecide()){
			...
			if (node.recCounter(node) == 1){
				...
				if (node.isLeaf()){
					...
					}else if (node.nodeRecNumber.get(node.getLeftChild()).equals(false) && !node.getLeftChild().getDecide()){
					...
					}else if (node.nodeRecNumber.get(node.getRightChild()).equals(false) && !node.getRightChild().getDecide()){
